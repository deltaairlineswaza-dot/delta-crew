...
response. That page is not generated by this bot. Wait a few minutes and
restart the service, check [Discord Status](https://discordstatus.com/), and
confirm that the hosting provider can make outbound HTTPS connections. The bot
replaces that HTML with a concise diagnostic so it does not flood the logs.
