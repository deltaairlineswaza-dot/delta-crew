...
`./scripts/render-build.sh` as the **Build Command** and
`./scripts/render-start.sh` as the **Start Command**. These checked-in scripts
install `requirements.txt` and launch `bot.py`, so the commands in Render stay
...
If the logs contain HTML comments such as `error-footer` or
`cf-error-details`, Discord returned a Cloudflare error page instead of an API
response. That page is not generated by this bot. If it persists, check
[Discord Status](https://discordstatus.com/) and confirm that the hosting
provider can make outbound HTTPS connections. The bot
replaces that HTML with a concise diagnostic so it does not flood the logs. It
also retries with an increasing delay (up to five minutes) instead of exiting
and causing Render to restart it repeatedly. The PyNaCl and `davey` messages
immediately before this error are unrelated warnings; this bot does not connect
to voice, so those optional voice dependencies are not required.
